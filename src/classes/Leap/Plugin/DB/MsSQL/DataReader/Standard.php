...
namespace Leap\Plugin\DB\MsSQL\DataReader {
	...
	class Standard extends \Leap\Core\DB\SQL\DataReader\Standard {
		...
		public function __construct(\Leap\Core\DB\Connection\Driver $connection, \Leap\Core\DB\SQL\Command $sql, $mode = NULL) {
			...
			$command = @mssql_query($sql->text, $resource);
			if ($command === FALSE) {
				throw new \Leap\Core\Throwable\SQL\Exception('Message: Failed to query SQL statement. Reason: :reason', array(':reason' => @mssql_get_last_message()));
			}
