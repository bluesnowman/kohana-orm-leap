...
namespace Leap\Core\DB\SQL\Tokenizer\Token {
	...
	/**
	 * This class represents the rule definition for a "dot" token, which the tokenizer will use
	 ...
	 * @version 2014-05-11
	 */
	class Dot extends \Leap\Core\DB\SQL\Tokenizer\Token {
		...
		public function process(&$statement, &$position, $strlen) {
			...
			if ($char == '.') {
				$tuple = array(
					'type' => \Leap\Core\DB\SQL\Tokenizer\TokenType::dot(),
					'token' => $char,
				);
				$position++;
				// var_dump($char);
