...
namespace Leap\Core\DB\ORM\Relation {
	 ...
	 * @version 2014-01-28
	 */
	class BelongsTo extends \Leap\Core\DB\ORM\Relation {
		...
		public function __construct(\Leap\Core\DB\ORM\Model $model, Array $metadata = array()) {
			...
			$parent_model = \Leap\Core\DB\ORM\Model::model_name($metadata['parent_model']);
		...
		protected function load() {
			...
			$parent_source = $parent_model::data_source(\Leap\Core\DB\DataSource::SLAVE_INSTANCE);

			$child_key = $this->metadata['child_key'];

			$builder = \Leap\Core\DB\SQL::select($parent_source)
				->all("{$parent_table}.*")
				->from($parent_table);

			$field_count = count($child_key);
			for ($i = 0; $i < $field_count; $i++) {
				$builder->where("{$parent_table}.{$parent_key[$i]}", \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, $this->model->{$child_key[$i]});
			}
