...
namespace Leap\Plugins\DB\Firebird {
	 ...
	 * @access public
	 * @class
	 * @package Leap\Plugins\DB\Firebird
	 * @version 2014-04-24
	 */
	class Schema extends \Leap\Core\DB\Schema {
		...
		public function fields($table, $like = '') {
			...
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->column(\Leap\Core\DB\SQL::expr('TRIM("RDB$INDICES"."RDB$RELATION_NAME")'), 'table')
				->column(\Leap\Core\DB\SQL::expr('TRIM("RDB$RELATION_FIELDS"."RDB$FIELD_NAME")'), 'column')
				->column(\Leap\Core\DB\SQL::expr('CASE "RDB$FIELDS"."RDB$FIELD_TYPE"
						WHEN 7 THEN
				...
				END'), 'type')
				->column(\Leap\Core\DB\SQL::expr('COALESCE("RDB$RELATION_FIELDS"."RDB$FIELD_POSITION", 0) + 1'), 'seq_index')
				->column(\Leap\Core\DB\SQL::expr('CASE COALESCE("RDB$RELATION_FIELDS"."RDB$NULL_FLAG", 0) WHEN 0 THEN 1 ELSE 0 END'), 'nullable')
				->column(\Leap\Core\DB\SQL::expr('SUBSTRING(CAST("RDB$RELATION_FIELDS"."RDB$DEFAULT_SOURCE" AS VARCHAR(255)) FROM 9)'), 'default')
				...
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'RDB$CHARACTER_SETS')
				...
				->where(\Leap\Core\DB\SQL::expr('TRIM("RDB$RELATION_FIELDS"."RDB$RELATION_NAME")'), \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, $table)
				->where(\Leap\Core\DB\SQL::expr('COALESCE("RDB$INDICES"."RDB$SYSTEM_FLAG", 0)'), \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 0)
				->order_by('RDB$RELATION_FIELDS.RDB$FIELD_POSITION');

			if ( ! empty($like)) {
				$builder->where(\Leap\Core\DB\SQL::expr('TRIM("RDB$RELATION_FIELDS"."RDB$FIELD_NAME")'), \Leap\Core\DB\SQL\Operator::_LIKE_, $like);
			}
		...
		public function indexes($table, $like = '') {
			...
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				->column(\Leap\Core\DB\SQL::expr("'{$schema}'"), 'schema')
				->column(\Leap\Core\DB\SQL::expr('TRIM("RDB$INDICES"."RDB$RELATION_NAME")'), 'table')
				->column(\Leap\Core\DB\SQL::expr('TRIM("RDB$INDICES"."RDB$INDEX_NAME")'), 'index')
				->column(\Leap\Core\DB\SQL::expr('TRIM("RDB$INDEX_SEGMENTS"."RDB$FIELD_NAME")'), 'column')
				->column(\Leap\Core\DB\SQL::expr('CAST(("RDB$INDEX_SEGMENTS"."RDB$FIELD_POSITION" + 1) AS integer)'), 'seq_index')
				->column(\Leap\Core\DB\SQL::expr('0'), 'ordering')
				->column(\Leap\Core\DB\SQL::expr('RDB$INDICES.RDB$UNIQUE_FLAG'), 'unique')
				->column(\Leap\Core\DB\SQL::expr('IIF("RDB$RELATION_CONSTRAINTS"."RDB$CONSTRAINT_TYPE" = \'PRIMARY KEY\', 1, 0)'), 'primary')
				->from('RDB$INDEX_SEGMENTS')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'RDB$INDICES')
				->on('RDB$INDICES.RDB$INDEX_NAME', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 'RDB$INDEX_SEGMENTS.RDB$INDEX_NAME')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'RDB$RELATION_CONSTRAINTS')
				...
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("RDB$INDICES"."RDB$RELATION_NAME")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("RDB$INDICES"."RDB$INDEX_NAME")'))
				->order_by(\Leap\Core\DB\SQL::expr('CAST(("RDB$INDEX_SEGMENTS"."RDB$FIELD_POSITION" + 1) AS integer)'));

			if ( ! empty($like)) {
				$builder->where(\Leap\Core\DB\SQL::expr('TRIM("RDB$INDICES"."RDB$INDEX_NAME")'), \Leap\Core\DB\SQL\Operator::_LIKE_, $like);
			}
		...
		public function tables($like = '') {
			...
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				->column(\Leap\Core\DB\SQL::expr("'{$schema}'"), 'schema')
				->column(\Leap\Core\DB\SQL::expr('TRIM("RDB$RELATION_NAME")'), 'table')
				->column(\Leap\Core\DB\SQL::expr("'BASE'"), 'type')
				->from('RDB$RELATIONS')
				->where(\Leap\Core\DB\SQL::expr('COALESCE("RDB$SYSTEM_FLAG", 0)'), \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 0)
				->where('RDB$VIEW_BLR', \Leap\Core\DB\SQL\Operator::_IS_, NULL)
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("RDB$RELATION_NAME")'));

			if ( ! empty($like)) {
				$builder->where(\Leap\Core\DB\SQL::expr('TRIM("RDB$RELATION_NAME")'), \Leap\Core\DB\SQL\Operator::_LIKE_, $like);
			}
		...
		public function triggers($table, $like = '') {
			...
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->column(\Leap\Core\DB\SQL::expr("CASE 'RDB\$TRIGGER_TYPE' WHEN 1 THEN 'INSERT' WHEN 2 THEN 'INSERT' WHEN 3 THEN 'UPDATE' WHEN 4 THEN 'UPDATE' ELSE 'DELETE' END"), 'event')
				->column(\Leap\Core\DB\SQL::expr("CASE 'RDB\$TRIGGER_TYPE' & 2 WHEN 0 THEN 'AFTER' ELSE 'BEFORE' END"), 'timing')
				...
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("RDB$RELATION_NAME")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("RDB$TRIGGER_NAME")'))
				->order_by('RDB$TRIGGER_SEQUENCE');

			if ( ! empty($like)) {
				$builder->where(\Leap\Core\DB\SQL::expr('TRIM("RDB$TRIGGER_NAME")'), \Leap\Core\DB\SQL\Operator::_LIKE_, $like);
			}
