...
namespace Leap\Plugins\DB\Oracle {
	 ...
	 * @access public
	 * @class
	 * @package Leap\Plugins\DB\Oracle
	 * @version 2014-04-19
	 */
	class Schema extends \Leap\Core\DB\Schema {
		...
		public function indexes($table, $like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->column(\Leap\Core\DB\SQL::expr("CASE \"t0\".\"DESCEND\" WHEN 'Y' THEN 'DESC' ELSE 'ASC' END"), 'ordering')
				->column(\Leap\Core\DB\SQL::expr("CASE \"t2\".\"CONSTRAINT_TYPE\" WHEN 'P' THEN 1 WHEN 'U' THEN 1 ELSE 0 END"), 'unique')
				->column(\Leap\Core\DB\SQL::expr("CASE \"t2\".\"CONSTRAINT_TYPE\" WHEN 'P' THEN 1 ELSE 0 END"), 'primary')
				->from('SYS.ALL_IND_COLUMNS', 't0')
				//->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'SYS.ALL_INDEXES', 't1')
				//->on('t1.OWNER', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't0.INDEX_OWNER')
				//->on('t1.INDEX_NAME', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't0.INDEX_NAME')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'SYS.ALL_CONSTRAINTS', 't2')
				->on('t2.INDEX_OWNER', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't0.INDEX_OWNER')
				->on('t2.INDEX_NAME', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't0.INDEX_NAME')
				->where('t0.TABLE_NAME', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, $table)
				//->where('t1.STATUS', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 'VALID')
				->where('t2.STATUS', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 'ENABLED')
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("t0"."TABLE_OWNER")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("t0"."TABLE_NAME")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("t0"."INDEX_NAME")'))
				->order_by('t0.COLUMN_POSITION');

			if ( ! empty($like)) {
				$builder->where('t0.INDEX_NAME', \Leap\Core\DB\SQL\Operator::_LIKE_, $like);
			}
		...
		public function tables($like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->from('SYS.ALL_TABLES')
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("OWNER")'))
		...
		public function triggers($table, $like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->column(\Leap\Core\DB\SQL::expr("CASE UPPER(\"TRIGGER_TYPE\") WHEN 'BEFORE EACH ROW' THEN 'BEFORE' WHEN 'BEFORE STATEMENT' THEN 'BEFORE' WHEN 'BEFORE EVENT' THEN 'BEFORE' WHEN 'AFTER EACH ROW' THEN 'AFTER' WHEN 'AFTER STATEMENT' THEN 'AFTER' WHEN 'AFTER EVENT' THEN 'AFTER' ELSE NULL END"), 'timing')
				->column(\Leap\Core\DB\SQL::expr("CASE UPPER(\"TRIGGER_TYPE\") WHEN 'BEFORE STATEMENT' THEN 'STATEMENT' WHEN 'AFTER STATEMENT' THEN 'STATEMENT' WHEN 'BEFORE EVENT' THEN 'EVENT' WHEN 'AFTER EVENT' THEN 'EVENT' ELSE 'ROW' END"), 'per')
				...
				->where('STATUS', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 'ENABLED')
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("OWNER")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("TABLE_NAME")'))
		...
		public function views($like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->from('SYS.ALL_VIEWS')
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("OWNER")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("VIEW_NAME")'));

			if ( ! empty($like)) {
				$builder->where('VIEW_NAME', \Leap\Core\DB\SQL\Operator::_LIKE_, $like);
			}
