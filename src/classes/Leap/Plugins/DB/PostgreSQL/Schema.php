...
namespace Leap\Plugins\DB\PostgreSQL {
	 ...
	 * @access public
	 * @class
	 * @package Leap\Plugins\DB\PostgreSQL
	 * @version 2014-04-19
	 */
	class Schema extends \Leap\Core\DB\Schema {
		...
		public function fields($table, $like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->column(\Leap\Core\DB\SQL::expr('GREATEST(character_maximum_length, numeric_precision, datetime_precision)'), 'max_length')
				->column(\Leap\Core\DB\SQL::expr('COALESCE(numeric_scale, 0)'), 'max_decimals')
				->column(\Leap\Core\DB\SQL::expr("''"), 'attributes')
				->column('max_length', 'seq_index')
				->column(\Leap\Core\DB\SQL::expr("CASE WHEN is_nullable = 'YES' THEN 1 ELSE 0 END"), 'nullable')
		...
		public function indexes($table, $like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->column(\Leap\Core\DB\SQL::expr('NULL'), 'ordering')
				->column(\Leap\Core\DB\SQL::expr("CASE \"t1\".\"indisunique\" WHEN 't' THEN 1 ELSE 0 END"), 'unique')
				->column(\Leap\Core\DB\SQL::expr("CASE \"t1\".\"indisprimary\" WHEN 't' THEN 1 ELSE 0 END"), 'primary')
				->from('pg_class', 't0')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'pg_index', 't1')
				->on('t1.indrelid', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't0.oid')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'pg_class', 't2')
				->on('t2.oid', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't1.indexrelid')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'pg_attribute', 't3')
				...
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, \Leap\Core\DB\SQL::expr('(SELECT "t5"."table_schema" AS "schname", "t5"."table_schema" AS "relname" FROM "information_schema"."tables" AS "t5" WHERE "t5"."table_type" = \'BASE TABLE\' AND "t5"."table_schema" NOT IN (\'pg_catalog\', \'information_schema\'))'), 't4')
				...
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("t4"."schname")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("t0"."relname")'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER("t2"."relname")'))
				->order_by('t3.attnum');
