...
namespace Leap\Plugins\DB\MsSQL {
	...
	class Schema extends \Leap\Core\DB\Schema {
		...
		public function indexes($table, $like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->from('SYS.TABLES', 't0')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'SYS.SCHEMAS', 't1')
				->on('t1.SCHEMA_ID', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't0.SCHEMA_ID')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'SYS.INDEXES', 't2')
				->on('t2.OBJECT_ID', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 't0.OBJECT_ID')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'SYS.INDEX_COLUMNS', 't3')
				...
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, 'SYS.COLUMNS', 't4')
				...
				->order_by(\Leap\Core\DB\SQL::expr('UPPER([t1].[NAME])'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER([t0].[NAME])'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER([t2].[NAME])'))
		...
		public function tables($like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->where(\Leap\Core\DB\SQL::expr("OBJECTPROPERTY(OBJECT_ID([TABLE_NAME]), 'IsMsShipped')"), \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 0)
		...
		public function triggers($table, $like = '') {
			$builder = \Leap\Core\DB\SQL::select($this->data_source)
				...
				->column(\Leap\Core\DB\SQL::expr("CASE WHEN OBJECTPROPERTY([t0].[ID], 'ExecIsInsertTrigger') = 1 THEN 'INSERT' WHEN OBJECTPROPERTY([t0].[ID], 'ExecIsUpdateTrigger') = 1 THEN 'UPDATE' WHEN OBJECTPROPERTY([t0].[ID], 'ExecIsDeleteTrigger') = 1 THEN 'DELETE' END"), 'event')
				->column(\Leap\Core\DB\SQL::expr("CASE WHEN OBJECTPROPERTY([t0].[ID], 'ExecIsInsteadOfTrigger') = 1 THEN 'INSTEAD OF' ELSE 'AFTER' END"), 'timing')
				...
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, '[SYS].[TABLES]', '[t3]')
				->on('[t3].[OBJECT_ID]', \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, '[t0].[PARENT_OBJ]')
				->join(\Leap\Core\DB\SQL\JoinType::_LEFT_, '[SYS].[SCHEMAS]', '[t4]')
				...
				->where(\Leap\Core\DB\SQL::expr("CASE WHEN OBJECTPROPERTY([t0].[ID], 'ExecIsTriggerDisabled') = 1 THEN 0 ELSE 1 END"), \Leap\Core\DB\SQL\Operator::_EQUAL_TO_, 1)
				->order_by(\Leap\Core\DB\SQL::expr('UPPER([t4].[NAME])'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER([t1].[NAME])'))
				->order_by(\Leap\Core\DB\SQL::expr('UPPER([t0].[NAME])'));
