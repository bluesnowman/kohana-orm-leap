...
namespace Leap\Plugin\DB\SQLite\DataReader {

	/**
	 * This class is used to read data from an SQLite database using the improved
	 * driver.
	 *
	 * @access public
	 * @class
	...
	class Improved extends \Leap\Core\DB\SQL\DataReader\Standard {
		...
		public function __construct(\Leap\Core\DB\Connection\Driver $connection, \Leap\Core\DB\SQL\Command $command, $mode = NULL) {
			...
			if ($handle === FALSE) {
				throw new \Leap\Core\Throwable\SQL\Exception('Message: Failed to query SQL command. Reason: :reason', array(':reason' => @$resource->lastErrorMsg()));
			}
			$this->handle = $handle;
			$this->record = FALSE;
		}
		...
		public function read() {
			if ($this->handle !== NULL) {
				$this->record = @$this->handle->fetchArray(SQLITE3_ASSOC);
			}
