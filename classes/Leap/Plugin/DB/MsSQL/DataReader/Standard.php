...
namespace Leap\Plugin\DB\MsSQL\DataReader {
	...
	class Standard extends \Leap\Core\DB\SQL\DataReader\Standard {
		...
		public function __construct(\Leap\Core\DB\Connection\Driver $connection, \Leap\Core\DB\SQL\Command $command, $mode = NULL) {
			...
			$handle = @mssql_query($command->text, $resource);
			if ($handle === FALSE) {
				throw new \Leap\Core\Throwable\SQL\Exception('Message: Failed to query SQL command. Reason: :reason', array(':reason' => @mssql_get_last_message()));
			}
			$this->handle = $handle;
			$this->record = FALSE;
		}
		...
		public function read() {
			$this->record = @mssql_fetch_assoc($this->handle);
