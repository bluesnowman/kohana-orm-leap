...
namespace Leap\Plugin\DB\Drizzle\DataReader {
	...
	class Standard extends \Leap\Core\DB\SQL\DataReader\Standard {
		...
		public function __construct(\Leap\Core\DB\Connection\Driver $connection, \Leap\Core\DB\SQL\Command $command, $mode = NULL) {
			...
			$handle = @mysql_query($command->text, $resource);
			if ($handle === FALSE) {
				throw new \Leap\Core\Throwable\SQL\Exception('Message: Failed to query SQL command. Reason: :reason', array(':reason' => @mysql_error($resource)));
			}
			$this->handle = $handle;
			$this->record = FALSE;
		}
		...
		public function read() {
			$this->record = @mysql_fetch_assoc($this->handle);
